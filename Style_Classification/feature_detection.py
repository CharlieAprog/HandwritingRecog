...
#get image dict
    archaic_imgs = {char:
                    [resize_pad(img, new_size_x, new_size_y) for img in get_style_char_images(style_base_path +'Archaic/' , char)]
                    for char in archaic_characters}
    hasmonean_imgs ={char:
                    [resize_pad(img, new_size_x, new_size_y) for img in get_style_char_images(style_base_path +'Hasmonean/', char)]
                    for char in hasmonean_characters}
    herodian_imgs = {char:
                     [resize_pad(img, new_size_x, new_size_y) for img in get_style_char_images(style_base_path +'Herodian/', char)]
    ...
    for image,label in zip(characters,labels):
        if (label == 21 or label == 22 or label == 13 or label == 11
            ...
            if idx2name[27] not in archaic_pdfs :
                archaic_pdfs[idx2name[27]] = get_hinge_pdf(idx2name[27], hasmonean_imgs)
