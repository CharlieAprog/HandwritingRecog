...
def getLines(newImage):
    ...
    return mid_lines, top_line, bottom_line
...
def plotImageAndHistLines(newImage, thr_peaks):
    figure = plt.figure(figsize=(16, 12))
    fs = 25
    plt.imshow(newImage)
    line_heights = []
    for i in range(len(thr_peaks)):
        for _ in thr_peaks[i].keys():
            if not thr_peaks[k]["lh"] <= outlier:
...
print(np.unique(image))
mid_lines, top_line, bottom_line = getLines(image)
mid_sections = getMidSections(mid_lines, image)
...
section = mid_sections[0]
offset_from_top = section[0]
print(np.unique(section))
...
# print(line_segments)
# plotImageAndLines(image, line_segments)
